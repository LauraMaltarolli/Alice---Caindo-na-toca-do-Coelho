...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.color = PLAYER_COLOR_NORMAL
        # Cria a imagem do jogador proceduralmente
        self._update_image_size()  # Isso cria self.image e self.rect
        self.rect.centerx = SCREEN_WIDTH // 2
        self.rect.centery = PLAYER_START_Y  # Posição Y fixada
        self.speed = 7
        # Controle de power-up
        self.effect = None
        self.effect_timer = 0
        self.effect_duration = 5000  # 5 segundos
    ...
    def _update_image_size(self):
        """Função-chave: Redesenha a imagem do jogador com base no 'self.radius' e 'self.color' atuais."""
        center = self.rect.center if hasattr(self, 'rect') else (0, 0)
        # Criar nova superfície e desenhar círculo
    ...
    def shrink(self):
        """Aplica o efeito 'Beba-me'."""
        self.radius = self.base_radius // 2
        self.color = PLAYER_COLOR_SHRINK
        self.effect = 'shrink'
    ...
    def grow(self):
        """Aplica o efeito 'Coma-me'."""
        self.radius = self.base_radius * 2
        self.color = PLAYER_COLOR_GROW
        self.effect = 'grow'
        self.effect_timer = pygame.time.get_ticks()
    ...
    def reset_size(self):
        """Reseta o tamanho e cor ao normal."""
        self.radius = self.base_radius
        self.color = PLAYER_COLOR_NORMAL
        self.effect = None
        self.effect_timer = 0
...
class TunnelObject(pygame.sprite.Sprite):
    def __init__(self, obj_type, speed):
        ...
        if self.obj_type == 'danger':
            width, height, self.color = random.randint(40, 100), random.randint(15, 30), RED
            self.is_circle = False
        elif self.obj_type == 'fake':
            width, height, self.color = random.randint(40, 100), random.randint(15, 30), BLUE
            self.is_circle = False
        elif self.obj_type == 'shrink':
            width, height, self.color = 28, 28, GREEN
            self.is_circle = True
        elif self.obj_type == 'grow':
            width, height, self.color = 32, 32, PURPLE
            self.is_circle = True
        # Criar a imagem e o rect proceduralmente
        self.image = pygame.Surface((width, height), pygame.SRCALPHA)
        if self.is_circle:
            pygame.draw.circle(self.image, self.color, (width // 2, height // 2), width // 2)
        else:
            self.image.fill(self.color)
        self.rect = self.image.get_rect()
        self.rect.x = random.randint(0, SCREEN_WIDTH - width)
        self.rect.y = SCREEN_HEIGHT + random.randint(20, 150)  # Spawn abaixo da tela
...
class Game:
    ...
    def run_game_loop(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == self.SPAWN_OBJECT_EVENT:
                ...
                spawn_interval = max(300, 1500 - int(self.score / 5))
        ...
        for hit in hits:
            if hit.obj_type == 'danger':
                ...
                self.particles.add(Particle(hit.rect.center, BLUE, min_speed=0.5, max_speed=2, size=3, num_particles=5))
    ...
    def show_game_over_screen(self):
        ...
        self.screen.fill(BLACK)
        self.draw_text("GAME OVER", self.font_main, RED, SCREEN_WIDTH / 4)
        if self.score > self.high_score:
